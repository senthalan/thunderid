...
const (
	...
	// validPermissionCharacters defines the allowed characters for handles and delimiters.
	// Allowed: a-z A-Z 0-9 . _ : - /
	validPermissionCharacters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789._:-/"
)
...
// validateDelimiter validates delimiter is a single valid permission character.
func validateDelimiter(delimiter string) *serviceerror.ServiceError {
	if len(delimiter) != 1 {
		return &ErrorInvalidDelimiter
	}
	if !isValidPermissionCharacter(rune(delimiter[0])) {
		return &ErrorInvalidDelimiter
	}
	return nil
}
